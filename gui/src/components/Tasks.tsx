...
import { isTaskRunning, useCancelTask, useTasks } from "../api/tasks";
...
export function Tasks(): JSX.Element {
  ...
  if (tasks.length === 0) {
    return <p className="px-2 py-6 text-center text-sm text-muted">Nothing is running in the background.</p>;
  }
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center gap-2">
        ...
        <span className="shrink-0 text-xs text-muted">{STATE_LABELS[task.state]}</span>
