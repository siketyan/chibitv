...
pub enum Table {
    Cdt(Cdt),
    /// DSM-CC download sections (header and CRC retained).
    Dsmcc(Vec<u8>),
    Pat(Pat),
    Cat(Cat),
    Pmt(Pmt),
...
impl Table {
    pub fn read(bytes: &mut Bytes) -> Result<Self> {
        ...
        Ok(match table_id {
            0x3B | 0x3C => Self::Dsmcc(bytes.to_vec()),
            0xC8 => Self::Cdt(Cdt::read(bytes)?),
...
mod tests {
    ...
    fn reads_common_data_without_panicking_on_truncation() {
        let section = [
            200, 0xf0, 0x10, 0x12, 0x34, 0xc5, 1, 2, 0, 4, 1, 0xf0, 0, 0xab, 0xcd, 0, 0, 0, 0,
        ];
        let Table::Cdt(table) = Table::read(&mut Bytes::copy_from_slice(&section)).unwrap() else {
            panic!("CDT expected");
        };
