...
/// A database the broadcast schedule is kept in.
///
/// Everything the rest of the program needs from a database is here, so
/// another one is supported by implementing this trait and handing
/// [`open`] its URL scheme.
#[async_trait]
...
pub async fn open(url: &str) -> anyhow::Result<Arc<dyn EventStore>> {
    match url.split_once(':') {
        Some(("sqlite", _)) => Ok(Arc::new(SqliteEventStore::open(url).await?)),
        Some((scheme, _)) => bail!("`{scheme}` is not a database chibitv can store events in"),
...
pub async fn restore(store: &Arc<dyn EventStore>, registry: &registry::Registry) -> usize {
